	...
	for (var i = 0, len = tpl.length; i < len; i++) {
		...
		if (context.disableParseTag) {
			...
			if (text === '{{') {
				...
				if (e > i) {
					flush();
					context.astStack.push(parseOutput(trim(tpl.slice(i + 2, e)), context));
					i = e + 1;
				}
			} else if (text === '{%') {
				var e = textIndexOf(tpl, '%}', i);
				if (e > i) {
					...
					if (e2 !== -1) {
						if (trim(strTmp.slice(e2 + 1)) === '') {
							strTmp = strTmp.slice(0, e2 + 1);
						}
					}
					// parse tag
					flush();
					parseTag(context, trim(tpl.slice(i + 2, e)));
					i = e + 1;
					// optimize: trim right
					var e3 = tpl.indexOf('\n', i + 1);
					if (e3 !== -1) {
						if ((trim(tpl.slice(i + 1, e3 + 1)) === '')) {
							i = e3;
							context.line++;
							context.lineStart = i;
						}
...
var baseTags = {
	...
	'section': function(context, name, body) {
		var blocks = arrayRemoveEmptyString(splitText(body, [' ']));
		var filename, id, locals;

		function get_section_id(name) {
			var i = 0;
			var id = name;

			while (global.sliquid.sections.indexOf(id) !== -1) {
				i++;
				id = name + '-' + i;
			}

			global.sliquid.sections.push(id);

			return id;
		}

		if (blocks.length === 1 && !(blocks[0].substr(0, 2) === '{{' && blocks[0].substr(-2) === '}}')) {
			// filename is a string
			filename = stripQuoteWrap(blocks[0]).trim();
		} else {
			// syntax error
			context.astStack.push(context.astNode(OPCODE.PRINTSTRING, '{% section ' + body + ' %}'));
			return;
		}

		id = get_section_id(filename);

		context.astStack.push(context.astNode(OPCODE.SECTION, filename, id));
	},

	'schema':	function(context, name, body) {
		var blocks	= arrayRemoveEmptyString(splitText(body, [' ']));
		var name	= genRandomName();

		context.astStack.newChild(context.astNode(OPCODE.SCHEMA, name));
	},

	'endschema':	function(context, name, body) {
		context.astStack.close();
	},

	'layout': function (context, name, body) {
		var blocks = arrayRemoveEmptyString(splitText(body, [' ']));

		if (blocks.length === 0) {
			// syntax error
			context.astStack.push(context.astNode(OPCODE.PRINTSTRING, '{% layout ' + body + ' %}'));
			return;
		}
		...
		context.astStack.push(context.astNode(OPCODE.LAYOUT, filename));
	},

	'paginate': function (context, name, body) {

	},

	'endpaginate': function (context, name, body) {

	},

	'form': function (context, name, body) {
		var blocks = arrayRemoveEmptyString(splitText(body, [' ']));
		var name = blocks[0];

		context.astStack.push(context.astNode(OPCODE.PRINTSTRING, '<form name="' + name + '">'));
	},

	'endform': function (context, name, body) {
		context.astStack.push(context.astNode(OPCODE.PRINTSTRING, '</form>'));
	}
};
...
var parseTag = function (context, text) {
	...
	if (i === -1) {
		...
		var name = text.slice(0, i).replace(/^\s+|\s+$/g, '');
