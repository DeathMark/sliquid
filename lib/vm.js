...
function getLocals(fullName, mainName, childs, context, callback, a1, a2, a3) {
	...
	if (/.*\[.*\]/.test(fullName)) {
		...
		for (i = 0; i < parts.length; i++) {
			childs = parts[i].split('.');

			if (i == 0) {
				info = getChildValue(context._locals, childs);
				...
				if (parseInt(parts[i]).toString() == parts[i] && info.hasOwnProperty(parts[i])) {
					info = info[parts[i]];
				} else {
					var v = getChildValue(context._locals, childs);

					if (v[0] && info.hasOwnProperty(v[1])) {
						info	= info[v[1]];
					} else {
						info	= null;
						break;
				...
				info = getChildValue(info, childs.length > 1 ? childs.slice(1) : null);
		...
		callback(null, info || null, a1, a2, a3);
		return;
	}
...
execOpcode[OPCODE.FORLOOPLOCALS] = function (context, callback, ast) {
	...
	if (loop) {
		switch (ast[1]) {
			case OPCODE.LOOPLOCALS_LENGTH:
				val = loop.length;
				break;
			case OPCODE.LOOPLOCALS_NAME:
				val = loop.itemName;
				break;
			case OPCODE.LOOPLOCALS_INDEX0:
				val = loop.index;
				break;
			case OPCODE.LOOPLOCALS_INDEX:
				val = loop.index + 1;
				break;
			case OPCODE.LOOPLOCALS_RINDEX0:
				val = loop.length - loop.index - 1;
				break;
			case OPCODE.LOOPLOCALS_RINDEX:
				val = loop.length - loop.index;
				break;
			case OPCODE.LOOPLOCALS_FIRST:
				val = loop.index < 1;
				break;
			case OPCODE.LOOPLOCALS_LAST:
				val = loop.index + 1 >= loop.length;
				break;
			default:
	...
	if (loop) {
		switch (ast[1]) {
			...
			case OPCODE.LOOPLOCALS_LAST:
				val = loop.index + 1 >= loop.length;
				break;
			case OPCODE.LOOPLOCALS_COL0:
				val = loop.colIndex;
				break;
			case OPCODE.LOOPLOCALS_COL:
				val = loop.colIndex + 1;
				break;
			case OPCODE.LOOPLOCALS_COL_FIRST:
				val = loop.colIndex < 1;
				break;
			case OPCODE.LOOPLOCALS_COL_LAST:
				val = loop.colIndex + 1 >= loop.columns;
				break;
			default:
...
execOpcode[OPCODE.KEYVAL]	= function(context, callback, ast) {
	callback(null, [ast[1], ast[2]]);
};

execOpcode[OPCODE.SECTION]	= function(context, callback, ast) {
	run(ast[1], context, function (err, filename) {
		if (err) {
			return callback(err);
		}
		...
		var id = ast[2], locals;
		var default_id	= id.replace(/-[0-9]+$/, '');

		var file	= global.sliquid.file;

		if (context._locals._sections) {
			if (context._locals._sections.hasOwnProperty(id)) {
				locals	= context._locals._sections[id];
			} else if (context._locals._sections._pages && context._locals._sections._pages.hasOwnProperty(file) && context._locals._sections._pages[file].hasOwnProperty(id)) {
				locals	= context._locals._sections._pages[file][id];
			} else if (context._locals._sections.hasOwnProperty(default_id)) {
				locals	= context._locals._sections[default_id];
			}
		}

		if (locals && locals.blocks) {
			locals.blocks.size = Object.keys(locals.blocks).length;

		}

		context.section(filename, {section: locals}, callback);
	});
};

execOpcode[OPCODE.SCHEMA]	= function(context, callback, ast) {
	callback(null);
};

execOpcode[OPCODE.LAYOUT] = function (context, callback, ast) {
	run(ast[1], context, function (err, filename) {
		if (err) {
			return callback(err);
		}
