...
/**
 * Include a section file
 *
 * @param {String} name
 * @param {Array} localsAst
 * @param {Function} callback
 */
Context.prototype.section = function (name, localsAst, callback) {
	if (typeof localsAst === 'function') {
		callback = localsAst;
		localsAst = null;
	}
	...
	if (typeof(this._includeFileHandler) === 'function') {
		this._includeFileHandler(name, function (err, astList) {
			...
			function start() {
				c.run(astList, function (err) {
					me.print('<div id="shopify-section-' + (localsAst && localsAst.section && localsAst.section.id ? localsAst.section.id : '') + '" class="shopify-section' + (localsAst && localsAst.section && localsAst.section.class ? ' ' + localsAst.section.class : '') + '">');
					me.print(c.clearBuffer());
					me.print('</div>');
					callback(err);
				});
